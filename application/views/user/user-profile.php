					...
					<div class="js-tabs my-account-tab" data-aos="fade-left" data-aos-duration="2400">
						...
						<div class='panels'>
							...
							<div id='tab2' class='content <?php echo ($this->session->flashdata('slidetab')==2)? 'active':''?>'>  
								...
								<div class="basic-info mt50">
									...
									<div class="text-center WaitingforFundApproval"> Your documents are being review and should be processed shortly. You will be notified once the verification process is complete.</div>
											...
											?>
									<div class="text-center WaitingforFundApproval"><i class="fa fa-check" aria-hidden="true" style="font-size:80px;"></i><h1>Verified</h1> Your Identity is verified.</div>
											<?php
										} else {
											?>
								...
								<div class="basic-info mt50">
									...
									<div class="text-center WaitingforFundApproval">
										<i class="fa fa-check" aria-hidden="true" style="font-size:80px;"></i><h1>Verified</h1> Your proof of fund is verified.
									...
									?>
										...
										<?php
											//if($userDetails['identity_proof_status'] != 2 && $userDetails['identity_proof_status'] != 1) {
												?>
											<!--<input type="hidden" name="personainquiryId" id="personainquiryId" value="">-->
								<div class="btn_center">
									<button type="button" id="proofoffundsubmitbtnprofile" class="btn"<?php echo ($userDetails['identity_proof_status'] < 1) ? ' disabled="true"' : ''; ?>>Submit</button>
								</div> 
												<?php
											//}
										?>
								<div class="basic-info mt50">
									<form id="verifyDigitalSignature" action="<?php echo base_url();?>user/uploadDigitalSignature" method="post">
										<input type="hidden" class="csrftoken" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>" />
										<h2 class="heading">Upload Digital Signature</h2>
										<ul class="">
											<li>Make sure any images you upload are crisp and not blurry.</li>
											<li>Do not cover up any part of the identification with your fingers.</li>
										</ul>
										<p>To enhance trust and protect all parties involved in a deal, we need to verify your digital signature. Please take a moment to upload your digital signature so that we can verify you while signing any NDA of listing.</p>
										<input type="file" name="upload_digital_signature" id="upload_digital_signature" />
										<div class="btn_center">
											<button type="submit" id="uploadDigitalSignBtn" class="btn" disabled="true">Upload</button>
										</div>
										<?php 
											if(!empty($digiSign)){
												?>
										<div class="text-center WaitingforFundApproval">
											<h4>Your Uploaded Digital Signature</h4>
											<div class="imageVal">
												<img src="<?php echo base_url(); ?>/uploads/user_digital_signature/<?php echo $digiSign['documents']; ?>" />
											</div>
										</div>
												<?php
											}
							...
							<div id='tab4' class='content' style="min-height:500px">  
								...
								<div class="investor-pass-para">You currently don't have a Investor Pass. Please contact
									<a href="mailto: <?php echo $permission['sitesettings']['helpline_email_address']?>"><?php echo $permission['sitesettings']['helpline_email_address']?></a> for more information.
								</div>
								<?php
								}
								?>
