			...
			while($Months[$i]){
				if($nval['year']<$currentYear || ($nval['year'] == $currentYear && $currentMonth > ($i+1)))
				{
					if($nval['avgProfit'][$i]!='' && $nval['avgRevenue'][$i]!='')
					{
						...
						$avgProfitArr[] = ($nval['avgProfit'][$i]!='')?$nval['avgProfit'][$i]:0;
						$avgRevenueArr[] = ($nval['avgRevenue'][$i]!='')?$nval['avgRevenue'][$i]:0;
						$a++;
					}
			...
			while($Months[$i]){
				if($nval['year']<$currentYear || ($nval['year'] == $currentYear && $currentMonth > ($i+1)))
				{
					if($nval['avgPageview'][$i]!='' && $nval['avgVisitor'][$i]!='')
					{
						...
						$avgPageviewArr[] = ($nval['avgPageview'][$i]!='')?$nval['avgPageview'][$i]:0;
						$avgVisitorArr[] = ($nval['avgVisitor'][$i]!='')?$nval['avgVisitor'][$i]:0;
					}
