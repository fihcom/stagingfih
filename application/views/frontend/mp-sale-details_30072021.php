			...
			<div class="container">
				...
				<div class="multiple-price">
					<ul class="ul row">
						...
						<li class="col-sm-2">
							<div class="box">
								<span><?php echo $pricing_period;?> Months</span>
								<p>Pricing Period</p>
							</div>
						</li>
						<li class="col-sm-2">
		...
		foreach($newEarningArr as $nval)
		{
			...
			while($Months[$i]){
				if($nval['year']<$currentYear || ($nval['year'] == $currentYear && $currentMonth > ($i+1)))
				{
					...
					$avgProfitArr[] = ($nval['avgProfit'][$i]>0)?$nval['avgProfit'][$i]:0;
					$avgRevenueArr[] = ($nval['avgRevenue'][$i]>0)?$nval['avgRevenue'][$i]:0;
				}
		...
		foreach($newTrafficArr as $nval)
		{
			...
			while($Months[$i]){
				if($nval['year']<$currentYear || ($nval['year'] == $currentYear && $currentMonth > ($i+1)))
				{
					...
					$avgPageviewArr[] = ($nval['avgPageview'][$i]>0)?$nval['avgPageview'][$i]:0;
					$avgVisitorArr[] = ($nval['avgVisitor'][$i]>0)?$nval['avgVisitor'][$i]:0;
				}
								...
								<div class="col-md-8 latest-list-middleCol">
									<a href="<?php echo base_url();?>listing/<?php echo $rb['listing_id'];?>" class="d-block">
										...
										<div class="starting-price">
											...
											<span class="multiple"><img src="<?php echo base_url();?>assets/frontend/images/multiple.png" alt=""><span><?php echo $rb['multiple'];?>x Multiple</span></span>
										</div>
									</a>
								</div>
