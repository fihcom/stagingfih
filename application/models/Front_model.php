...
class Front_model extends CI_Model
{
    ...
    {
        $this->db->select('BaseTbl.userId,BaseTbl.Status', FALSE);
    ...
    {
        $this->db->select('BaseTbl.userId, BaseTbl.password, BaseTbl.fname AS name, BaseTbl.lname AS lastname, BaseTbl.role, BaseTbl.mail, BaseTbl.phone, BaseTbl.authorize', FALSE);
        $this->db->from(TABLE_PREFIX.'users as BaseTbl', FALSE);
        $this->db->where('BaseTbl.mail', $email);
    ...
    function userForgetPass($email)
    {
        ...
        $this->db->from(TABLE_PREFIX.'users as BaseTbl', FALSE);
        $this->db->where('BaseTbl.mail', $email);
        //$this->db->where('BaseTbl.oauth_provider',NULL);
        $this->db->where('BaseTbl.Status', 1);
        $this->db->limit(1);
        $query = $this->db->get();        
        $user = $query->row();
    ...
    function getlendingcontent(){
        $query = $this->db->query("select c.* from ".TABLE_PREFIX."acquisition_lending_contents c where c.admin_id=1");
        return $contents = $query->row();
    }
    ...
    function getUserUnlockedListing($options){
        ...
        $groupBy = 'BaseTbl.listing_id,BaseTbl.listing_id, BaseTbl.monetization, BaseTbl.traffic_per_month, BaseTbl.userId, BaseTbl.Status, BaseTbl.business_create_date, BaseTbl.monthly_profit, BaseTbl.monthly_revenue, BaseTbl.price, BaseTbl.listing_id, BaseTbl.business_image, BaseTbl.multiple, industryname, countryname, buyer, seller, tempRec.data_json, BaseTbl.date_added';
        $q = "select SQL_CALC_FOUND_ROWS BaseTbl.monetization,BaseTbl.traffic_per_month,BaseTbl.userId,BaseTbl.Status,BaseTbl.business_create_date,BaseTbl.monthly_profit,BaseTbl.monthly_revenue,BaseTbl.price,BaseTbl.listing_id,BaseTbl.business_image,BaseTbl.multiple, BaseTbl.pricing_period AS revenue_multiple,ind.industry as industryname,C.name as countryname,br.userId as buyer,br.sellerId as seller,tempRec.data_json from 
        ...
        join ".TABLE_PREFIX."sell_business_temp as tempRec on tempRec.id=BaseTbl.listing_temp_id
        ...
        where h.activity='UNCOVER' and h.userId=$userId and (BaseTbl.Status=1 OR BaseTbl.Status=4 OR BaseTbl.Status=3) ".$partSql." order by BaseTbl.date_added desc limit ".$start.",".$limit.""; // group by ".$groupBy."

        /* 
            select SQL_CALC_FOUND_ROWS BaseTbl.monetization,BaseTbl.date_added, BaseTbl.traffic_per_month,BaseTbl.userId,BaseTbl.Status,BaseTbl.business_create_date,BaseTbl.monthly_profit,BaseTbl.monthly_revenue,BaseTbl.price,BaseTbl.listing_id,BaseTbl.business_image,BaseTbl.multiple,ind.industry as industryname,C.name as countryname,br.userId as buyer,br.sellerId as seller,tempRec.data_json from npb_investor_pass_history as h join npb_sell_record as BaseTbl on h.unlocked_business=BaseTbl.listing_id join npb_sell_business_temp as tempRec on tempRec.id=BaseTbl.listing_temp_id join npb_industries as ind ON ind.id = BaseTbl.industry left join npb_countries C ON C.id=BaseTbl.country left join npb_buy_request br ON br.listing_id=BaseTbl.listing_id and br.status=2 where h.activity='UNCOVER' and h.userId=81 and (BaseTbl.Status=1 OR BaseTbl.Status=4 OR BaseTbl.Status=3) group by BaseTbl.listing_id, BaseTbl.monetization, BaseTbl.traffic_per_month, BaseTbl.userId, BaseTbl.Status, BaseTbl.business_create_date, BaseTbl.monthly_profit, BaseTbl.monthly_revenue, BaseTbl.price, BaseTbl.listing_id, BaseTbl.business_image, BaseTbl.multiple, industryname, countryname, buyer, seller, tempRec.data_json, BaseTbl.date_added order by BaseTbl.date_added desc limit 0,10;
        */
    ...
    function getMarketplaceListing($options){
        ...
        $Monitization = $options['Monitization'];
        $businessAge = $options['businessAge'];
        $listingCountry = $options['listingCountry'];
        $priceMin = $options['min'];
        $priceMax = $options['max'];
        $searchText = $options['searchText'];
        ...
        if($businessAge != '')
        {
            ...
            /* == Sourav Das Work - 08042025 == */
            if($businessAge == '2lessyears') {
                $partSql .= " and BaseTbl.business_create_date >= DATE_SUB(NOW(), INTERVAL 2 YEAR)";
            } elseif($businessAge == '2-5yrs') {
                $partSql .= " and BaseTbl.business_create_date >= DATE_SUB(NOW(), INTERVAL 5 YEAR) and BaseTbl.business_create_date <= DATE_SUB(NOW(), INTERVAL 2 YEAR)";
            } elseif($businessAge == '5-10yrs') {
                $partSql .= " and BaseTbl.business_create_date >= DATE_SUB(NOW(), INTERVAL 10 YEAR) and BaseTbl.business_create_date <= DATE_SUB(NOW(), INTERVAL 5 YEAR)";
            } elseif($businessAge == '10-20yrs') {
                $partSql .= " and BaseTbl.business_create_date >= DATE_SUB(NOW(), INTERVAL 20 YEAR) and BaseTbl.business_create_date <= DATE_SUB(NOW(), INTERVAL 10 YEAR)";
            } elseif($businessAge == '20moreyears') {
                $partSql .= " and BaseTbl.business_create_date <= DATE_SUB(NOW(), INTERVAL 20 YEAR)";
            }
        }
        if($priceMin != '')
        {
        ...
        /* == Sourav Das 10042025 == */
        if($trafficFrom != '') {
            $partSql .= " and BaseTbl.pricing_period >= $trafficFrom";
        }
        if($trafficTo != '') {
            $partSql .= " and BaseTbl.pricing_period <=$trafficTo";
        }
        if($show_home == 'Y')
        {
        ...
        /* == Sourav Das - 09042025 == */
        if($listingCountry != ''){
            $partSql .= " and C.sortname = '$listingCountry'";
        }

        // $groupBy = 'BaseTbl.monetization, BaseTbl.userId, BaseTbl.Status, BaseTbl.business_create_date, BaseTbl.monthly_profit, BaseTbl.monthly_revenue, BaseTbl.price, BaseTbl.listing_id, BaseTbl.business_image, BaseTbl.multiple, BaseTbl.traffic_per_month, industryname, countryname, buyer, br.sellerId, BaseTbl.date_added';
        $q = "select SQL_CALC_FOUND_ROWS BaseTbl.monetization,BaseTbl.userId,BaseTbl.Status,BaseTbl.business_create_date,BaseTbl.monthly_profit,BaseTbl.monthly_revenue,BaseTbl.price,BaseTbl.listing_id,BaseTbl.business_image, BaseTbl.pricing_period AS revenue_multiple,BaseTbl.multiple,BaseTbl.traffic_per_month,ind.industry as industryname,C.name as countryname,br.userId as buyer,br.sellerId as seller from 
        ".TABLE_PREFIX."sell_record as BaseTbl
        join ".TABLE_PREFIX."industries as ind ON ind.id = BaseTbl.industry
        left join ".TABLE_PREFIX."countries C ON C.id=BaseTbl.country
        left join ".TABLE_PREFIX."buy_request br ON br.listing_id=BaseTbl.listing_id and br.status=2
        where (BaseTbl.Status=1 OR BaseTbl.Status=4 OR BaseTbl.Status=3) ".$partSql." order by BaseTbl.date_added desc limit ".$start.",".$limit."";
        $query = $this->db->query($q);   // group by BaseTbl.listing_id 
        ...
        if(is_array($listing) && count($listing)>0 && $detaillisting == true)
        {
            ...
            foreach($listing as $kk=>$vv)
            {
                ...
                $listing[$l]['price_formated'] = number_format($vv['price'],0,'.',',');
                $l++;
            }
        }
    ...
    function getMinMaxDetails(){
        ...
        $q = "select max(BaseTbl.price) as maxPrice, min(BaseTbl.price) as minPrice, max(BaseTbl.monthly_revenue) as maxRevenue, min(BaseTbl.monthly_revenue) as minRevenue ,max(BaseTbl.monthly_profit) as maxProfit, min(BaseTbl.monthly_profit) as minProfit, max(BaseTbl.multiple) as maxMultiple, min(BaseTbl.multiple) as minMultiple, max(BaseTbl.pricing_period) as maxRevenueMultiple, min(BaseTbl.pricing_period) as minRevenueMultiple, min(BaseTbl.traffic_per_month) as minTraffic, max(BaseTbl.traffic_per_month) as maxTraffic from 
        ".TABLE_PREFIX."sell_record as BaseTbl
        where (BaseTbl.Status=1 OR BaseTbl.Status=4 OR BaseTbl.Status=3) limit 0,1";
        $query = $this->db->query($q);    
        $manmin = $query->row();
    ...
    function getRecentListing($options){
        ...
        $q = "select SQL_CALC_FOUND_ROWS BaseTbl.monetization,BaseTbl.userId,BaseTbl.Status,BaseTbl.business_create_date,BaseTbl.monthly_profit,BaseTbl.monthly_revenue,BaseTbl.price,BaseTbl.listing_id,BaseTbl.business_image,BaseTbl.multiple, BaseTbl.pricing_period AS revenue_multiple, BaseTbl.traffic_per_month,ind.industry as industryname,C.name as countryname,br.userId as buyer,br.sellerId as seller from 
        ".TABLE_PREFIX."sell_record as BaseTbl
        join ".TABLE_PREFIX."industries as ind ON ind.id = BaseTbl.industry
        left join ".TABLE_PREFIX."countries C ON C.id=BaseTbl.country
        left join ".TABLE_PREFIX."buy_request br ON br.listing_id=BaseTbl.listing_id and br.status=2
        where (BaseTbl.Status=1 OR BaseTbl.Status=4 OR BaseTbl.Status=3) and BaseTbl.show_home ='Y' order by BaseTbl.date_added desc limit ".$start.",".$limit.""; // group by BaseTbl.listing_id
    ...
    function getSellerListing($options){
        ...
        // Group by statement segregated for FULL_GROUP_BY_MODE permission
        $groupBy = 'BaseTbl.listing_id, BaseTbl.monetization, BaseTbl.traffic_per_month, BaseTbl.userId, BaseTbl.Status, BaseTbl.business_create_date, BaseTbl.monthly_profit, BaseTbl.monthly_revenue, BaseTbl.price, BaseTbl.business_image, BaseTbl.multiple, industryname, countryname, buyer, br.sellerId, BaseTbl.date_added';
        ...
        left join ".TABLE_PREFIX."countries C ON C.id=BaseTbl.country
        left join ".TABLE_PREFIX."buy_request br ON br.listing_id=BaseTbl.listing_id and br.status=2
        where  (BaseTbl.Status=1 OR BaseTbl.Status=4 OR BaseTbl.Status=3) and BaseTbl.userId=".$userId." ".$partSql." order by BaseTbl.date_added desc limit ".$start.",".$limit.""; // group by ".$groupBy."
    ...
    public function getSellerSoldListing($arr){
        $groupBy = 'BaseTbl.monetization, BaseTbl.traffic_per_month, BaseTbl.traffic_per_month, BaseTbl.userId, BaseTbl.Status, br.transfer_status, BaseTbl.business_create_date, BaseTbl.monthly_profit, BaseTbl.monthly_revenue, BaseTbl.price, BaseTbl.listing_id, BaseTbl.business_image, BaseTbl.multiple, industryname, countryname, salestatus, br.buy_amt_stranfer_status, buyer, seller, lo.offer_price, BaseTbl.date_added';

        $q = "select SQL_CALC_FOUND_ROWS BaseTbl.monetization,BaseTbl.traffic_per_month,BaseTbl.traffic_per_month,BaseTbl.userId,BaseTbl.Status,br.transfer_status,BaseTbl.business_create_date,BaseTbl.monthly_profit,BaseTbl.monthly_revenue,BaseTbl.price,BaseTbl.listing_id,BaseTbl.business_image,BaseTbl.multiple, BaseTbl.pricing_period AS revenue_multiple,ind.industry as industryname,C.name as countryname,br.status as salestatus,br.buy_amt_stranfer_status,br.userId as buyer,br.sellerId as seller,lo.offer_price from 
        ".TABLE_PREFIX."sell_record as BaseTbl
        join ".TABLE_PREFIX."industries as ind ON ind.id = BaseTbl.industry
        join ".TABLE_PREFIX."buy_request as br ON br.listing_id=BaseTbl.listing_id
        left join ".TABLE_PREFIX."countries C ON C.id=BaseTbl.country
        ...
        where BaseTbl.Status=4 and br.status=2 and br.sellerId=".$arr['userId']." order by BaseTbl.date_added desc"; // group by ".$groupBy."
        $query = $this->db->query($q);    
        $listing = $query->result_array();
    ...
    public function getlendinglist(){
        $this->db->select("a.*,a.industry as industryname");
        $this->db->from(TABLE_PREFIX.'acquisition_lending as a');
        $this->db->where('a.status', 1); 
        $this->db->order_by('a.Added_on', 'desc');   
        $Query = $this->db->get();      
        $lendingList = $Query->result_array();
        return $lendingList;
    }

    public function checkListingUncovered($listingId, $userId){
        $this->db->select('h.*', FALSE);
        $this->db->from(TABLE_PREFIX.'investor_pass_history as h', FALSE);
        $this->db->where('h.userId', $userId);
        $this->db->where('h.activity', 'UNCOVER');
    ...
    /* == Sourav Das - 09-04-2025 == */
    public function getAllCountries(){
        $this->db->distinct();
        $this->db->select('C.id, C.name, C.sortname');
        $this->db->from(TABLE_PREFIX.'countries AS C', FALSE);
        $this->db->join(TABLE_PREFIX.'sell_record AS SR', 'C.id = SR.country', 'right');
        $this->db->where('C.sortname IS NOT NULL');
        $this->db->where('SR.status', '1');
        $this->db->or_where('SR.status', '3');
        $this->db->or_where('SR.status', '4');
