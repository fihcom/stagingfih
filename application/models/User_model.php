...
class User_model extends CI_Model
{
    ...
    function Addgetfunded($arr,$action = 'add')
    {
        if($action == 'add')
        {
            $this->db->insert(TABLE_PREFIX.'get_fund_list', $arr);
            return $this->db->insert_id();
        }
    }

    function AddRequestAccess($arr,$action = 'add')
    {
        if($action == 'add')
        {
            $this->db->insert(TABLE_PREFIX.'request_access_list', $arr);
            return $this->db->insert_id();
        }
    }
    function Addfundacquisition($arr,$action = 'add')
    {
        if($action == 'add')
        {
            $this->db->insert(TABLE_PREFIX.'fund_acquisition', $arr);
            return $this->db->insert_id();
        }
    }
    ...
    public function getpromocodedetails($promocode,$clientId=0){
        ...
        if($promoDetails->id >0)
        {
            $promoDetails->promotype = 'ADMIN';
        }else{
            $loggedInUser = $this->session->userdata('user_id');
            //$q = "select userId from ".TABLE_PREFIX."users u left join ".TABLE_PREFIX."sell_business_temp sbt on sbt.userId=$loggedInUser and sbt.data_json like '%\"promocode\": \"$promocode\"%' and sbt.Status>0  where promo_code='$promocode' and Status=1 and userId<>$loggedInUser and sbt.id is null";
            //$q = "select u.userId from ".TABLE_PREFIX."users u where u.promo_code='$promocode' and u.Status=1 and u.userId<>$loggedInUser";
            $q = "select u.userId as id from ".TABLE_PREFIX."users u left join ".TABLE_PREFIX."sell_business_temp sbt on sbt.userId='$clientId' and sbt.data_json like '%\"promocode\":\"$promocode\"%' and sbt.Status>0 where u.promo_code='$promocode' and u.Status=1 and u.userId<>'$clientId' and sbt.id IS NULL";
            $query = $this->db->query($q);    
            $promoDetails = $query->row();
            if($promoDetails->id>0)
            {
                //$q = "select tmp.id from ".TABLE_PREFIX."sell_business_temp tmp where ";
                if($promoDetails->id>0)
                {

                    $promoDetails->promotype = 'USER';
                    $promoDetails->promocode = $promocode;
                    $promoDetails->discount_type = 'Amount';
