...
class Admin_model extends CI_Model
{
    ...
    function getPassHistoryByListing($listingId) {
        $this->db->select("CONCAT_WS(' ', USR.fname, USR.lname) AS contact_name, USR.mail, PHS.userId, PHS.date_unlocked, PHS.unlocked_business, PHS.signed_nda_if_any");
        $this->db->from(TABLE_PREFIX . 'investor_pass_history AS PHS');
        $this->db->join(TABLE_PREFIX . 'users AS USR', 'USR.userId = PHS.userId', 'left');
        $this->db->where('PHS.unlocked_business', $listingId);
        $this->db->where('PHS.activity', 'UNCOVER');
        $this->db->order_by('id', 'desc');
        $Query = $this->db->get();
    ...
    function getpendingfaq($arr, $sort)
    {
        ...
        left join " . TABLE_PREFIX . "users as u on u.userId=f.userId
        left join " . TABLE_PREFIX . "sell_record as sr on sr.listing_id=f.listing_id
        left join " . TABLE_PREFIX . "sell_business_temp as sbt on sbt.listing_id=f.listing_id
        left join " . TABLE_PREFIX . "users as u1 on u1.userId=sr.userId
        " . $partSql . " " . $sortsql . " limit " . $arr['start'] . "," . $arr['limit'] . "";
        $query = $this->db->query($q);
    ...
    function getwalletbuyreqadmin($arr, $sort = [])
    {
        ...
        if ($arr['searchName'] != '') {
            $partSql .= " and (u.fname like '%" . $arr['searchName'] . "%' || u.lname like '%" . $arr['searchName'] . "%' || f.transaction_ref like '%" . $arr['searchName'] . "%' || f.description like '%" . $arr['searchName'] . "%' || f.listing_id like '%" . $arr['searchName'] . "%')";
        }
        if ($arr['id'] > 0) {
            $partSql .= " and f.id=" . $arr['id'];
        }
    ...
    {
        $this->db->select("*");
        $this->db->from(TABLE_PREFIX . 'banner_settings');
        $Query = $this->db->get();
        $Array = $Query->row();
        return $Array;
    }
    ...
    function getcalllog($arr)
    {
        ...
        //$arr['limit'] = 10;.
        if ($arr['user'] > 0) {
            $partSql = " and t.user_id=" . $arr['user'];
        }
    ...
    public function alertbanner($arr, $section)
    {
        ...
        if ($section == 'add') {
            ...
            unset($arr['submit']);
            if ($arr['area'] != 'inside') {
                $arr['homelistcontents'] = json_encode($arr['homelistcontents']);
            }
            unset($arr['area']);
            $this->db->insert(TABLE_PREFIX . 'banner_settings', $arr);
        } elseif ($section == 'edit') {
            $id = $arr['id'];
            unset($arr['action']);
            unset($arr['submit']);
            ...
            unset($arr['area']);
            $this->db->where('id', $id);
            $this->db->update(TABLE_PREFIX . 'banner_settings', $arr);
        }
    }

    function updateLendingContentDetails($editArray, $adminID, $detailsArr = array())
    {
        if (is_array($editArray) && count($editArray) > 0) {
            $this->db->where('admin_id', $adminID);
            $this->db->update(TABLE_PREFIX . 'acquisition_lending_contents', $editArray);
        }
        return $this->db->affected_rows();
    }
    function getLendingContentDetails($adminID)
    {
        $this->db->select("*");
        $this->db->from(TABLE_PREFIX . 'acquisition_lending_contents');
        $Query = $this->db->get();
        $Array = $Query->row();
    ...
    // Uncover count
    function countUncoveredListingsByUser($userID){
        $this->db->select("*");
        $this->db->from(TABLE_PREFIX . 'investor_pass_history');
        $this->db->where('userId', $userID);
        $this->db->where('activity', 'UNCOVER');
        $Query = $this->db->get();
        // echo $this->db->last_query();
    ...
    // Uncover count
    function getUncoveredListingsByUser($userID){
        $this->db->select("IPH.*, LIST.id as mainlistingId");
        $this->db->from(TABLE_PREFIX . 'investor_pass_history AS IPH');
        $this->db->join(TABLE_PREFIX . 'sell_business_temp AS LIST', 'LIST.listing_id = IPH.unlocked_business', 'left');
        $this->db->where('IPH.userId', $userID);
        $this->db->where('IPH.activity', 'UNCOVER');
        $Query = $this->db->get();
        return $Query->result_array();
    }

    function getUncoveredListByID($listingId){
        $this->db->select("IPH.*, LIST.id as mainlistingId, LIST.data_json as dJson");
        $this->db->from(TABLE_PREFIX . 'investor_pass_history AS IPH');
        $this->db->join(TABLE_PREFIX . 'sell_business_temp AS LIST', 'LIST.listing_id = IPH.unlocked_business', 'left');
        $this->db->where('IPH.unlocked_business', $listingId);
        $this->db->where('IPH.activity', 'UNCOVER');
        $Query = $this->db->get();
        return $Query->row_array();
    }

    function getUserDigiSign($userId) {
        //$user['userId'];
        $this->db->select('*', FALSE);
        $this->db->from(TABLE_PREFIX.'proof_of_funds', FALSE);
        $this->db->where('userId', $userId);    
        $this->db->where('doc_type', 'digisign');
        $Query = $this->db->get();
        $Array = $Query->row();
        return $Array;
    }

    function getExportedUserData() {
        $retSQL = 'CONCAT_WS(" ", U.fname, U.lname) AS full_name, U.mail AS email, U.phone AS phone_number, U.dob, CASE WHEN UD.proof_fund_status = 1 THEN \'Approved\' ELSE \'Rejected\' END AS fund_status, DATE_FORMAT(UD.fund_proof_approve_date, "%d/%m/%y") AS fund_proof_approve_date, CASE WHEN UD.identity_proof_status = 1 THEN \'Approved\' ELSE \'Rejected\' END AS identity_proof_text, DATE_FORMAT(UD.identity_proof_approve_date, "%d/%m/%y") AS identity_proof_approve_date, CASE WHEN U.Status = 1 THEN \'Pending\' WHEN U.Status = 2 THEN \'Rejected\'
        ELSE \'Approved\' END AS statusText, DATE_FORMAT(U.added_on, "%d/%m/%y") AS added_on';
        $this->db->select($retSQL, FALSE);
        $this->db->from(TABLE_PREFIX . 'users U', FALSE);
        $this->db->join(TABLE_PREFIX . 'user_details UD', 'U.userId = UD.userId', 'left');
