...
class Marketplace extends REST_Controller {
    ...
    public function mpListing_post(){
        ...
        $arr['listingCountry'] = $this->security->xss_clean(trim($data['listingCountry']));
        ...
        /* == Sourav Das - 10042025 == */
        $arr['TrafficFrom'] = $this->security->xss_clean(trim($data['TrafficFrom']));
        $arr['TrafficTo'] = $this->security->xss_clean(trim($data['TrafficTo']));
    ...
    public function requestfinancing_post(){
        $data = $this->input->post();
        $config = [
            [
                'field' => 'Email',
        ...
        $user_subject = 'A site user raised a request for financing details for the listing #'.$data['listingNo'].' - FIH.com';
        ...
        <br>
        <p style="padding: 8px 0; margin: 0 0 0 0; font: normal 16px arial">A site user raised a request for financing details for the listing #'.$data['listingNo'].'. The information is given below.<br> Listing: <a href="'.base_url().'listing/'.$data['listingNo'].'">'.$data['listingNo'].'</a><br> Email: '.$data['Email'].'<br>Phone: '.$data['Phone'].'</p>
        <br>
        ...
        $this->response([
            'status' => TRUE,
            'message' => 'Thanks for showing your interest. Our team will contact you as soon as possible.',
            'data' => $data
        ], REST_Controller::HTTP_OK);
    }

    public function becometheceo_post(){
        $data = $this->input->post();
        $config = [
            [
                'field' => 'Email',
        ...
        $user_to = $site_settings['support_email_address'];
        //$user_to = 'surajit@eclick.co.in';
        $user_subject = 'A site user raised Become the CEO  for the listing #'.$data['listingNo'].' - FIH.com';

        $user_message = '<h6 style="padding: 5px 0; margin: 0 0 0 0; font: normal 17px arial">Dear <strong style="padding: 0 0 0 0; margin: 0 0 0 0;  font: bold 16px arial;">Admin</strong>,</h6>
        <br>
        <p style="padding: 8px 0; margin: 0 0 0 0; font: normal 16px arial">A site user raised Become the CEO  for the listing #'.$data['listingNo'].'. The information is given below.<br>Listing: <a href="'.base_url().'listing/'.$data['listingNo'].'">'.$data['listingNo'].'</a><br> Email: '.$data['Email'].'<br>Phone: '.$data['Phone'].'<br>LinkedIn Profile: '.$data['LinkedInProfile'].'</p>
        <br>
        <p style="padding: 8px 0; margin: 0 0 0 0; font: normal 16px arial;">Cheers,</p>
        <p style="padding: 8px 0; margin: 0 0 0 0; font: normal 16px arial;">The Fih.com Team.</p>';
        $sendEmail = sendEmail($user_to, $user_subject, $user_message);
    ...
    public function lendingoffer_post(){
        ...
        $config = [
            ...
            [
                ...
                'rules' => 'trim|required',
                'errors' => [
                        'required' => 'Amount of money for lending is reqired.',
                ],
            ],[
                'field' => 'yearly_interest',
                'label' => 'yearly_interest',
                'rules' => 'trim|required',
                'errors' => [
                        'required' => 'Yearly interest is reqired.',
                ]
                ],[
                'field' => 'loan_term',
                'label' => 'loan_term',
        ...
        $user_to = $site_settings['support_email_address'];
        //$user_to = 'surajit@eclick.co.in';
        $user_subject = 'A site user raised lend money to an Accredited Acquirer to Buy this Successful Business #'.$data['listingNo'].' - FIH.com';
        if($data['provision'] == 'Y')
        {
            $provision = 'Business goes back into possession of lender upon acquirer default.';
        }else{
            $provision = 'Acquirer signs personal guarantee on loan.';
        }
        if($data['investor'] == 'Y')
        {
            $investor = 'Yes';
        ...
        $user_message = '<h6 style="padding: 5px 0; margin: 0 0 0 0; font: normal 17px arial">Dear <strong style="padding: 0 0 0 0; margin: 0 0 0 0;  font: bold 16px arial;">Admin</strong>,</h6>
        <br>
        <p style="padding: 8px 0; margin: 0 0 0 0; font: normal 16px arial">A site user raised lend money to an Accredited Acquirer to Buy this Successful Business #'.$data['listingNo'].'. 
        The information is given below.
        <br>Listing: <a href="'.base_url().'listing/'.$data['listingNo'].'">'.$data['listingNo'].'</a><br> 
        ...
        <strong>What is the maximum amount of money you would lend to an acquirer to purchase this business?</strong> <br>'.$data['money_lending'].'<br>
        <strong>What net yearly interest/yield do you require for the money you lend?</strong> <br>'.$data['yearly_interest'].'<br>
        <strong>How long of a term would you want your loan to be for?</strong> <br>'.$data['loan_term'].'<br>
        <strong>Which default provision do you require?</strong> <br>'.$provision.'<br>
        <strong>Any other relevant terms to mention?</strong> <br>'.$data['terms'].'<br>
        </p>
        <br>
        <p style="padding: 8px 0; margin: 0 0 0 0; font: normal 16px arial;">Cheers,</p>
        <p style="padding: 8px 0; margin: 0 0 0 0; font: normal 16px arial;">The Fih.com Team.</p>';
    ...
    public function listingaskquestion_post(){
        ...
        $user_to = $userDetails->mail;
        $user_subject = 'You received a question for listing #'.$arr['listing_id'].' - FIH.com';
        $user_message = '<h6 style="padding: 5px 0; margin: 0 0 0 0; font: normal 17px arial">Dear <strong style="padding: 0 0 0 0; margin: 0 0 0 0;  font: bold 16px arial;">'.$userDetails->fname.'</strong>,</h6>
        <br>
        <p style="padding: 8px 0; margin: 0 0 0 0; font: normal 16px arial">You received a question for listing #'.$arr['listing_id'].'. Please reply to it in the FAQ section of your seller dashboard.</p>
        <br>
        <p style="padding: 8px 0; margin: 0 0 0 0; font: normal 16px arial;">Cheers,</p>
        <p style="padding: 8px 0; margin: 0 0 0 0; font: normal 16px arial;">The Fih.com Team.</p>';
        $sendEmail = sendEmail($user_to, $user_subject, $user_message);
    ...
    /* == Sourav Das - 09-09-2025 == */
    public function countries_post(){
        $listdata = $this->front_model->getAllCountries();
