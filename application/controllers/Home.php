...
class Home extends CI_Controller {
    ...
    public function index($curPage=0) {
        ...
        */
        ...
        $url = 'api/data/bannercontents';
        $bannerdetails = sendRestRequest($url, []);
        $header['banner'] = $bannerdetails['data'];
    ...
    public function businessbrokerage(){
        $header = $this->front_model->header();
        $footer = $this->front_model->footer();
        $this->load->view('frontend/includes/inside-header',$header);
        $this->load->view('frontend/jotform');
        $this->load->view('frontend/includes/footer',$footer);
    }
    
    public function opensrs(){
        //$user_to = 'team@fih.com';
        //$user_to = 'chansuro@gmail.com';
        ...
        //     <br>
        //     <p style="padding: 8px 0; margin: 0 0 0 0; font: normal 16px arial">An application to sell business has been received and has been submitted for processing.  listing ID is #2345.</p>
        //     <br>
		...
		// $sendEmail = sendEmail($user_to, $user_subject, $user_message);
         ...
         $user_subject = 'Try 3 email from surajit';
            $user_message = '<h6 style="padding: 5px 0; margin: 0 0 0 0; font: normal 17px arial">Dear <strong style="padding: 0 0 0 0; margin: 0 0 0 0;  font: bold 16px arial;">Admin</strong>,</h6>
            <br>
            <p style="padding: 8px 0; margin: 0 0 0 0; font: normal 16px arial"> This is test email from Surajit</p>
            <br>
            <p style="padding: 8px 0; margin: 0 0 0 0; font: normal 16px arial;">Cheers,</p>
			<p style="padding: 8px 0; margin: 0 0 0 0; font: normal 16px arial;">The Fih.com Team.</p>';
            $sendEmail = sendEmail($user_to, $user_subject, $user_message);
    ...
    public function listingDetails($listingNo)
    {
        ...
        // NDA Content
        $getNDAContent = $this->admin_cms_model->getCMSPageDetailsbyslug('nda-content');
        $data['getNDAContent']['onlyNDAContent'] = $getNDAContent;
        ...
        $checkListingCover = $this->front_model->checkListingUncovered($listingNo, $this->session->userdata('user_id'));
        $data['checkListingCover'] = $checkListingCover;
    ...
    public function listingUncover(){
        if($this->session->userdata('isLoggedIn'))
		{
            ...
            if($res['data']['isunlocked'] == false && $res['data']['isidentityproofed'] == true && $res['data']['isfundproofed'] == true && $res['data']['isavailablefund'] == true) {
                ...
                // Check if user digital signature 
                // $getDigiSign = $this->admin_model->getUserDigiSign($this->session->userdata('user_id'));
                $getNDAContent = $this->admin_cms_model->getCMSPageDetailsbyslug('nda-content');
                $createPDF = createNDApdf($getNDAContent, $userDataPost['listingNo'], $this->session->userdata('user_id'), $userDataPost['hidden_image_sign']);
                ...
                $uncover['signed_nda_if_any'] = $createPDF;
    ...
    public function listingUncoverProcess($listingId){
        if(!$this->session->userdata('isLoggedIn') && !$this->session->userdata('user_id'))
		{
    ...
    public function marketPlace($curPage=0) {
        ...
        /*$userData['start'] = $page;
        $listdata = $this->front_model->getMarketplaceListing($userData);
        ...
        /* == Sourav Das - Filter Countries Data Options - 09-04-2025 == */
        $url = 'api/marketplace/countries';
		$userDataPost = [];
        $countriesData = sendRestRequest($url, $userDataPost);
        $dataPost['countriesData'] = $countriesData['data'];
    ...
    public function requestfinancingdetails(){
        // if($this->session->userdata('isLoggedIn'))
		// {
            $url = 'api/marketplace/requestfinancing';
            $userData = $this->input->post();
            $userData['user'] = $this->session->userdata('user_id');
            $res = sendRestRequest($url, $userData);
            header('Content-Type: application/json');
            $arr = array('response'=>$res,'token'=>$this->security->get_csrf_hash());
    ...
    public function becometheceo(){
            $url = 'api/marketplace/becometheceo';
            $userData = $this->input->post();
            $userData['user'] = $this->session->userdata('user_id');
            $res = sendRestRequest($url, $userData);
            header('Content-Type: application/json');
    ...
    public function lendingoffer(){
        $url = 'api/marketplace/lendingoffer';
        $userData = $this->input->post();
        $userData['user'] = $this->session->userdata('user_id');
        $res = sendRestRequest($url, $userData);
        header('Content-Type: application/json');
    ...
    public function financing(){
        $header = $this->front_model->header();
        $footer = $this->front_model->footer();
        $url = 'api/data/getlendinglist';
        $res = sendRestRequest($url, []);
        $data['lendinglist']= $res;
		...
		$this->load->view('frontend/includes/inside-header',$header);
		$this->load->view('frontend/financing',$data);
		$this->load->view('user/includes/footer',$footer);
    }

    public function financingrequestaccess(){
        
        $url = 'api/data/financingrequestaccess';
		$userDataPost = $this->input->post();
        $res = sendRestRequest($url, $userDataPost);
    ...
    public function financinggetfund(){
        
        $url = 'api/data/financinggetfund';
		$userDataPost = $this->input->post();
        $res = sendRestRequest($url, $userDataPost);
        $res['token'] = $this->security->get_csrf_hash();
        header('Content-Type: application/json');
    ...
    public function financingfundacquisition(){
        
        $url = 'api/data/financingfundacquisition';
		$userDataPost = $this->input->post();
        $res = sendRestRequest($url, $userDataPost);
        $res['token'] = $this->security->get_csrf_hash();
        header('Content-Type: application/json');
    ...
    public function cmsLinks($linkslug){
		...
		$this->load->view('frontend/includes/inside-header',$header);
        if($data['page_details']['pageSlug'] == $linkslug)
        {
            if($linkslug == 'aboutus')
            {
    ...
    public function sell_your_business(){
		...
		$data['currency'] = $this->front_model->getSelectedCurreny($currencyID);
		$header = $this->front_model->header();
		$header['site_settings']->site_title = 'Sell your Business - '.$header['site_settings']->site_title;
        $footer = $this->front_model->footer();
		$this->load->view('user/includes/header',$header);
		$this->load->view('user/sell-your-business',$data);
		$this->load->view('user/includes/footer',$footer);
	}

	public function sell_your_business_action(){

		$url = 'api/data/update-sell-your-business';
		$userDataPost = $this->input->post();
		$userDataPost['clientId'] = $this->session->userdata('user_id');
		$res = sendRestRequest($url, $userDataPost);
	...
	public function applyPromo(){
		...
		$url = 'api/data/applypromo';
		$userDataPost = $this->input->post();
		$userDataPost['clientId'] = $this->session->userdata('user_id');
		$resRecord = sendRestRequest($url, $userDataPost);
		
		$this->session->set_flashdata('slidetab', 'submission');
		redirect(base_url().'user/sell-your-business');    
		
	}
	public function businessregpayment(){
		$url = 'api/data/selltemprecord';
		$userDataPost = $this->input->post();
		$userDataPost['clientId'] = $this->session->userdata('user_id');
        if($this->session->userdata('user_id')>0)
		...
		$resRecord = sendRestRequest($url, $userDataPost);
		$data_json = $resRecord['data']['tempdata']['data_json'];
		...
		delete_cookie('me_po'); 
        $sessionArray = array('userId'=>$this->session->userdata('userId'),
                            'name'=>$this->session->userdata('name'),
                            'lastLogin'=> '',
                            'isLoggedIn' => TRUE,
                            'admin_login_as_politician'=>'admin_login_as_politician',
                            'politician_owner_id'=>$this->session->userdata('user_id'),
        );
        
        $cookie = array(
            'name'   => 'me_po',
            'value'  => json_encode($sessionArray),
            'expire' => '1800',  // Two weeks
        );
        set_cookie($cookie);
		$sitesettings = $this->admin_model->getSiteSettings();
		//print '<pre>';
		//print_r($data_json_arr['promo_details']);
		//die;
		if(is_array($data_json_arr['promo_details']) && count($data_json_arr['promo_details'])>0)
		{
			$Price = $data_json_arr['promo_details']['Sell_amount_discounted'];
		}else{
			$Price = $sitesettings['sell_business_amount'];
		}
		if($Price >0)
		{
			$currencyID = $sitesettings['currency'];
			$currencyDetails = $this->front_model->getSelectedCurreny($currencyID);
			$paypalemail = $sitesettings['paypal_email'];
			$returnURL = base_url().'user/sell-your-business/success';
			$cancelURL = base_url().'user/sell-your-business/cancel';
			$notifyURL = base_url().'paypal/ipn';
			$this->paypal_lib->add_field('return', $returnURL);
			$this->paypal_lib->add_field('cancel_return', $cancelURL);
			$this->paypal_lib->add_field('notify_url', $notifyURL);
			$this->paypal_lib->add_field('item_name', 'Sell Business Registration Fih.com');
			$this->paypal_lib->add_field('custom', 'SELLBUSINESS:'.$this->session->userdata('user_id').':'.$resRecord['data']['tempdata']['id']);
			...
			$userDataPost['clientId'] = $this->session->userdata('userId');
			$res = sendRestRequest($url, $userDataPost);
			redirect(base_url().'user/sell-your-business/success');  
		}
